...
export async function sendInvoiceEmail({ to, clientName, invoiceId, reservationDate, creneau, articles, total }) {
  ...
  const rows = articles.map(a => `
    <tr>
      ...
      <td style="padding:8px 4px;border-bottom:1px solid #f0f0f0;text-align:right">${(parseFloat(a.price) * a.quantity).toFixed(2)} €</td>
    </tr>`).join('')
...
export async function sendConfirmationEmail({ to, clientName, reservationId, invoiceId, date, creneau, articles, total }) {
  ...
  const rows = articles.map(a => `
    <tr>
      ...
      <td style="padding:7px 4px;border-bottom:1px solid #f0f0f0;text-align:right">${(parseFloat(a.price) * a.quantity).toFixed(2)} €</td>
    </tr>`).join('')
...
export async function sendAdminNotification({ reservationId, clientName, date, creneau, articles, total }) {
  const adminEmail = process.env.ADMIN_EMAIL || process.env.SMTP_USER
  if (!adminEmail) {
    console.log(`[ADMIN NOTIF] Nouvelle réservation ${reservationId} - ${clientName} - ${date} ${creneau} - ${total} €`)
    return { simulated: true }
  }

  const from    = process.env.SMTP_FROM || "Andy's Mayotte <noreply@chezandys.com>"
  const subject = `[Nouvelle réservation] ${reservationId} - ${clientName}`

  const rows = (articles || []).map(a => `
    <tr>
      <td style="padding:6px 4px;border-bottom:1px solid #f0f0f0">${a.emoji || ''} ${a.name}</td>
      <td style="padding:6px 4px;border-bottom:1px solid #f0f0f0;text-align:center">${a.quantity} ${a.unit || ''}</td>
      <td style="padding:6px 4px;border-bottom:1px solid #f0f0f0;text-align:right">${(parseFloat(a.price) * a.quantity).toFixed(2)} €</td>
    </tr>`).join('')

  const html = `<!DOCTYPE html><html><body style="margin:0;font-family:Arial,sans-serif;background:#f9fafb">
    <div style="max-width:600px;margin:32px auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.08)">
      <div style="background:#1c4532;padding:24px 32px">
        <h1 style="margin:0;color:white;font-size:20px">Nouvelle réservation</h1>
        <p style="margin:6px 0 0;color:#6ee7b7;font-size:13px">Andy's Mayotte - Espace admin</p>
      </div>
      <div style="padding:28px 32px">
        <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;margin-bottom:20px">
          <p style="margin:0 0 6px;font-size:13px;color:#6b7280;text-transform:uppercase;font-weight:600">Réservation N°</p>
          <p style="margin:0 0 8px;font-size:20px;font-weight:700;color:#1c4532">${reservationId}</p>
          <p style="margin:0;font-size:14px;color:#374151"><strong>Client :</strong> ${clientName}</p>
          <p style="margin:4px 0 0;font-size:14px;color:#374151"><strong>Date :</strong> ${formatDateFR(date)} - <strong>Créneau :</strong> ${creneau}</p>
        </div>
        <table style="width:100%;border-collapse:collapse">
          ...
          <tfoot>
            <tr style="background:#f0fdf4">
              <td colspan="2" style="padding:10px 4px;font-weight:700;color:#1c4532">Total indicatif</td>
              <td style="padding:10px 4px;font-weight:700;color:#1c4532;text-align:right">${parseFloat(total).toFixed(2)} €</td>
            </tr>
          </tfoot>
        </table>
      </div>
      <div style="padding:14px 32px;background:#f9fafb;border-top:1px solid #e5e7eb;text-align:center">
        <p style="margin:0;font-size:10px;color:#9ca3af">Andy's Mayotte - Notification automatique</p>
      </div>
    </div>
  </body></html>`
  ...
  const transporter = getTransporter()
  if (!transporter) {
    console.log(`[EMAIL non configuré] Admin notif ${reservationId} → ${adminEmail}`)
    return { simulated: true }
  }
  return transporter.sendMail({ from, to: adminEmail, subject, html })
}
